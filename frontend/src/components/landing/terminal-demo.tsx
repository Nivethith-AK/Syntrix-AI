...
const CONFIGS = [
  {
    ...
    min: "0",
    max: "100",
    tools: '["eda", "train", "shap"]',
  },
  {
    name: "churn-model",
    runtime: "syntrix-agents",
    memory: "workspace",
    min: "1",
    max: "50",
    tools: '["train", "explain", "report"]',
  },
  {
    name: "eda-deep-dive",
    runtime: "syntrix-ml",
  ...
  },
  {
    ...
    min: "5",
    max: "500",
    tools: '["resume", "approve", "report"]',
  },
  {
    name: "board-report",
    runtime: "syntrix-report",
    memory: "dataset",
    min: "0",
    max: "25",
    tools: '["markdown", "pdf", "shap"]',
  },
]
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-12 lg:gap-20">
            ...
            >
              <div className="flex items-center justify-between border-b border-[var(--color-baltic-sea-800)] px-4 py-3 bg-[var(--color-baltic-sea-950)]/50">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-[var(--color-baltic-sea-500)] font-mono">workspace.config.yaml</span>
                </div>
              ...
              <div className="p-5 font-mono text-sm overflow-x-auto">
                <pre className="text-[var(--color-baltic-sea-400)]">
                  <code>
                    <span className="text-[var(--color-baltic-sea-500)]">{"# Define your workspace"}</span>
                    {"\n"}
                    <span className="text-[var(--color-keppel-400)]">export default</span>{" "}
                    <span className="text-[var(--color-baltic-sea-200)]">defineWorkspace</span>
                    {"({"}
                    {"\n"}
                    {"  "}
          ...
          <div className="lg:max-w-md">
            ...
            <p className="mt-4 text-lg text-[var(--color-baltic-sea-400)]">
              Create a project, upload a dataset, and let Syntrix agents handle EDA, training, and reports.
            </p>

            <div ref={bulletsSectionRef} className="mt-8 space-y-4">
              {["Free local demo workspace", "No credit card required", "Email or Google sign-in"].map(
                (text, index) => (
                  <div
                    key={index}
